...
export default class Comentar extends Component
{
	...
	comenta = () => 
	{
		if(this.state.typed_about  == '' )
		{
			...
			global.firebase.database().ref(global.databasePath+'messages/' + global.ver_perfil_id+'/' + global.userId + '/'+global.comment_Id).set({ 
                userId			: global.userId,
				name			: global.user.name,
				picture			: global.user.picture,
				to 				: global.ver_perfil.name,
				about 			: this.state.typed_about,
				created	        : new Date().toDateString(),
				timestamp       : global.firebase.database.ServerValue.TIMESTAMP
			});

			global.firebase.database().ref(global.databasePath+'messages/' + global.userId + '/'+ global.ver_perfil_id+'/' +global.comment_Id).set({ 
                userId			: global.userId,
				name			: global.user.name,
				picture			: global.user.picture,
				to 				: global.ver_perfil.name,
			...
			if(global.tipo_mensaje == 'profile'){
